...
```{r Chunk 10}
---
title: "Comparison of ZHVI Across Regions"
author: "Sagar Shah"
date: "2024-10-21"
output:
  html_document:
...
# 1. Load the dataset
data <- read.csv("/Users/sagarshah/Documents/Introduction To Data Science /zhvi-all-homes_md-va-dc_2021-2023.csv")

# Convert 'month' to date format
data$month <- as.Date(data$month, format="%Y-%m-%d")

# Check for missing ZHVI values
missing_data <- sum(is.na(data$ZHVI))
cat("Missing ZHVI values: ", missing_data, "\n")

# 2. Filter data to include only non-missing ZHVI values
data_filtered <- data[!is.na(data$ZHVI), ]
...
# 1. Load the dataset
data <- read.csv("~/Documents/Introduction To Data Science /zhvi-all-homes_md-va-dc_2021-2023.csv")

# 2. Convert 'month' to date format
data$month <- as.Date(data$month, format="%Y-%m-%d")
...
ggplot(data[!is.na(data$ZHVI),], aes(x = month, y = ZHVI)) +
...
```{r Chunk 12}
---
title: "Correlation between SizeRank and ZHVI"
author: "Sagar Shah"
date: "2024-10-21"
output:
  html_document:
...
ggplot(data, aes(x = SizeRank, y = ZHVI)) +
...
This scatter plot indicates a negative correlation between SizeRank and ZHVI, meaning larger areas generally have lower home values, while smaller ones tend to have higher values.
...
Our primary research question seeks to determine whether housing prices are rising in this region and what 
implications these trends hold for various demographics.
...
To better understand factors affecting home values in the provided dataset, additional information on property type (e.g., single-family homes, 
condos), the number of bedrooms, and property size would be beneficial. Including socioeconomic indicators, such as the average income levels 
of residents in each area, could offer insights into the affordability and demand drivers influencing the Zillow Home Value Index (ZHVI). Other 
useful data points might include crime rates, school district quality, and proximity to public transportation, as these factors often impact 
real estate values and demand. Weather/climate information could also provide context, especially for areas where extreme temperatures or 
natural disaster risks affect home desirability. Expanding the dataset with these variables would allow for a more detailed analysis of the 
correlations between home values and external factors, leading to a more comprehensive understanding of real estate market trends across 
different regions.

After conducting Exploratory Data Analysis (EDA), our research question evolved to focus more specifically on the affordability of housing in 
the DMV area, influenced by rising trends in home values. Initial investigations revealed significant discrepancies in home prices across 
different zip codes, prompting us to consider socio-economic factors that may impact affordability. The ZHVI data illustrated clear trends but 
also highlighted areas with considerable variability. This led us to refine our question to not only address price increases but also 
investigate how these changes affect different demographics and the overall housing market dynamics in the region. Additionally, we recognized \
the importance of examining external factors like interest rates and economic policies that could further influence housing trends. Ultimately, 
EDA underscored the necessity of integrating additional variables like income levels and property characteristics for a more comprehensive 
analysis. This shift allowed us to develop a more nuanced understanding of the housing market in the DMV area.

Based on exploratory data analysis (EDA), it appears that housing prices in the D.C., Maryland, and Virginia (DMV) areas are trending upward
from 2021 to 2023. The Zillow Home Value Index (ZHVI) suggests a consistent increase in home values across various zip codes, particularly 
within the 35th to 65th percentiles. However, the limitations of using Zillow’s data, such as reliance on properties listed on their platform 
and potential discrepancies in the Zestimate, indicate that while the data provides valuable insights, it may not fully represent the broader 
market dynamics. Factors like income levels, property characteristics, and local economic conditions could further elucidate the housing market 
trends in the DMV area. Addressing these variables through additional research could enhance the understanding of housing price fluctuations 
and affordability challenges in this region.

In conclusion, this exploratory data analysis of housing prices in the D.C., Maryland, and Virginia (DMV) 
area reveals significant upward trends in home values over the past few years. The dataset sourced from Zillow 
highlights the complexities of the housing market and underscores the need for localized analysis to understand 
the diverse factors influencing affordability and access to housing.

While the ZHVI data indicates a general rise in housing prices, it also points to considerable variability across 
different zip codes, suggesting that not all areas are experiencing these changes uniformly. The findings align 
with broader national trends, yet they also highlight the unique characteristics of the DMV market, influenced by 
local economic conditions and demographic shifts.
