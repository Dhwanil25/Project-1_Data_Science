...
```
So based on the results of the model. We have to test for overfitting. We have a mutliple r-squared of 0.932 and an adjusted r-squared of 0.93. It's possible that adding city overcomplicated the data since it is a factor variable with many levels. We will test that next
...
```{r}
hist(merged_data$log_averageincome, main = "Histogram of Average Income", xlab = "Average Income")

# Ensure zhvi is numeric
merged_data$zhvi <- as.numeric(merged_data$zhvi)


merged_data$log_of_ZHVI <- log(merged_data$ZHVI)


#Test which variable log(ZHVI) has a stronger relationship with 
cor(merged_data$log_of_ZHVI, merged_data$averageincome, use = "complete.obs")
cor(merged_data$log_of_ZHVI, log(merged_data$averageincome), use = "complete.obs")
...
Compared to the original linear model with only geographical predictors, this model is significantly improved. The Multiple R-squared is at 0.784 and the adjusted is the same. 
...
The multiple and adjusted r-squared are now 0.804.
