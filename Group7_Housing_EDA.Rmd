...
Based on this first model we can observe year does have a significant impact on the log of ZHVI. The coefficient of year being positive, we can see that log ZHVI will increase by 5.8% within each year of our data. 

Housing values in Maryland are lower than the reference state(DC) by approximately -15.8% (exp(0.0564) -1) (after accounting for the year and metro variables).

Housing values in Virginia are lower than the reference state(DC) by 12.9%. (exp(-0.0137)-1)

More importantly, we observe that Metro areas are highly significant due to p-values below 0.05. 
Coefficient values for specific metro areas have stronger affects in areas like Washington-Arlington-Alexandria, and Easton, MD. Others are significantly lower like in Big Stone Gap, VA
